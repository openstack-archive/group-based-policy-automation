...
servicechain_instance_template = '''
{
  "AWSTemplateFormatVersion" : "2010-09-09",
  "Description" : "Template to test neutron service chain instance",
  "Parameters" : {},
  "Resources" : {
    "servicechain_instance": {
      "Type": "OS::Neutron::ServiceChainInstance",
      "Properties": {
        "name": "test-sc-instance",
        "description": "test service chain instance resource",
        "servicechain_spec": "1234",
        "port_id": "7890",
        "config_params": "{'type': 'TAP'}"
      }
    }
  }
}
...
class ServiceChainInstanceTest(HeatTestCase):

    def setUp(self):
        super(ServiceChainInstanceTest, self).setUp()
        self.m.StubOutWithMock(gbpclient.Client,
                               'create_servicechain_instance')
        self.m.StubOutWithMock(gbpclient.Client,
                               'delete_servicechain_instance')
        self.m.StubOutWithMock(gbpclient.Client,
                               'show_servicechain_instance')
        self.m.StubOutWithMock(gbpclient.Client,
                               'update_servicechain_instance')
        self.stub_keystoneclient()

    def create_servicechain_instance(self):
        gbpclient.Client.create_servicechain_instance({
            "servicechain_instance": {
                "name": "test-sc-instance",
                "description": "test service chain instance resource",
                "servicechain_spec": "1234",
        ...
        }).AndReturn({'servicechain_instance': {'id': '5678'}})

        snippet = template_format.parse(servicechain_instance_template)
        stack = utils.parse_stack(snippet)
        resource_defns = stack.t.resource_definitions(stack)
        return servicechain.ServiceChainInstance(
            'servicechain_instance',
            resource_defns['servicechain_instance'], stack)

    def test_create(self):
        rsrc = self.create_servicechain_instance()
        self.m.ReplayAll()
        scheduler.TaskRunner(rsrc.create)()
        self.assertEqual((rsrc.CREATE, rsrc.COMPLETE), rsrc.state)
        self.m.VerifyAll()
    ...
    def test_create_failed(self):
        ...
        stack = utils.parse_stack(snippet)
        resource_defns = stack.t.resource_definitions(stack)
        rsrc = servicechain.ServiceChainInstance(
            'servicechain_instance',
            resource_defns['servicechain_instance'], stack)
    ...
    def test_delete(self):
        gbpclient.Client.delete_servicechain_instance('5678')
        gbpclient.Client.show_servicechain_instance('5678').AndRaise(
            servicechain.NeutronClientException(status_code=404))
    ...
    def test_delete_already_gone(self):
        gbpclient.Client.delete_servicechain_instance('5678').AndRaise(
            servicechain.NeutronClientException(status_code=404))
    ...
    def test_update(self):
        rsrc = self.create_servicechain_instance()
        gbpclient.Client.update_servicechain_instance(
            '5678', {'servicechain_instance': {'name': 'instance_update'}})
        self.m.ReplayAll()
        scheduler.TaskRunner(rsrc.create)()

        update_template = copy.deepcopy(rsrc.t)
        update_template['Properties']['name'] = 'instance_update'
        scheduler.TaskRunner(rsrc.update, update_template)()
