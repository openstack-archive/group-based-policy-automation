...
class ServiceChainInstance(gbpresource.GBPResource):

    PROPERTIES = (
        TENANT_ID, NAME, DESCRIPTION, SERVICECHAIN_SPEC, PORT_ID,
        CONFIG_PARAMS
    ) = (
        'tenant_id', 'name', 'description', 'servicechain_spec', 'port_id',
        'config_params'
    ...
    properties_schema = {
        TENANT_ID: properties.Schema(
            properties.Schema.STRING,
            _('Tenant id of the service chain instance.')
        ),
        NAME: properties.Schema(
            properties.Schema.STRING,
            _('Name of the service chain instance.'),
            update_allowed=True
        ),
        DESCRIPTION: properties.Schema(
            properties.Schema.STRING,
            _('Description of the service chain instance.'),
            update_allowed=True
        ),
        SERVICECHAIN_SPEC: properties.Schema(
            properties.Schema.STRING,
            _('Service chain spec of the service chain instance.'),
            required=True,
            update_allowed=True
        ),
        PORT_ID: properties.Schema(
            properties.Schema.STRING,
            _('Port id of the service chain instance.'),
            required=True,
            update_allowed=True
        ),
        CONFIG_PARAMS: properties.Schema(
            properties.Schema.STRING,
            _('Config params of this of this instance.'),
            update_allowed=True
        )
    }
    ...
    def _show_resource(self):
        client = self.grouppolicy()
        sc_instance_id = self.resource_id
        sc_instance = client.show_servicechain_instance(sc_instance_id)
        return sc_instance['servicechain_instance']
    ...
    def handle_create(self):
        ...
        sc_instance = client.create_servicechain_instance(
            {'servicechain_instance': props})['servicechain_instance']

        self.resource_id_set(sc_instance['id'])
    ...
    def handle_delete(self):
        ...
        try:
            client.delete_servicechain_instance(sc_instance_id)
        except NeutronClientException as ex:
            self.client_plugin().ignore_not_found(ex)
        else:
            return self._delete_task()
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            self.grouppolicy().update_servicechain_instance(
                self.resource_id, {'servicechain_instance': prop_diff})
